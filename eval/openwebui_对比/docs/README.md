...
本仓库包含两项 RAG 相关改造的评测与验证：

1. **独立 Rerank**（`rag.rerank_enabled`）：Rerank 不再依赖 Hybrid Search；
2. **追加式 RAG 上下文注入**（`rag.append_rag_context`）：不修改历史用户消息，
   同一轮工具调用不重复注入相同 Sources。

评测脚本、数据集与结果见 [eval/](eval/)，完整验证报告见 [VALIDATION.md](VALIDATION.md)。
